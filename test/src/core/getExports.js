...
describe('ExportMap', function () {
  ...
  it('returns a cached copy on subsequent requests', function () {
    expect(ExportMap.get('./named-exports', fakeContext)).to.exist.and.equal(
  ...
  it('does not return a cached copy with different settings', () => {
    ...
    expect(
      ExportMap.get('./named-exports', differentSettings),
    ).to.exist.and.not.to.equal(firstAccess)
  })
    ...
    function jsdocTests(parseContext, lineEnding) {
      describe('deprecated imports', function () {
        ...
        it('works with named imports.', function () {
          ...
          expect(imports.get('fn')).to.have.nested.property(
            'doc.tags[0].title',
            'deprecated',
          )
          expect(imports.get('fn')).to.have.nested.property(
  ...
  describe('Map API', function () {
    describe('#size', function () {
      it('counts the names', () =>
        expect(ExportMap.get('./named-exports', fakeContext)).to.have.property(
          'size',
          12,
        ))

      it('includes exported namespace size', () =>
        expect(ExportMap.get('./export-all', fakeContext)).to.have.property(
          'size',
          1,
        ))
    })
  ...
  describe('issue #210: self-reference', function () {
    it(`doesn't crash`, function () {
      expect(() => ExportMap.get('./narcissist', fakeContext)).not.to.throw(
        Error,
      )
    })
    it(`'has' circular reference`, function () {
      expect(ExportMap.get('./narcissist', fakeContext)).to.exist.and.satisfy(
        m => m.has('soGreat'),
      )
    })
    it(`can 'get' circular reference`, function () {
      expect(ExportMap.get('./narcissist', fakeContext)).to.exist.and.satisfy(
        m => m.get('soGreat') != null,
      )
    })
  })
