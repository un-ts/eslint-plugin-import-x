...
describe('resolve', function () {
  ...
  it('resolves via a custom resolver with interface version 2', function () {
    ...
    expect(replaceErrorStackForTest(testContextReports[0].message)).to.equal(
  ...
  it('reports invalid i/resolver config', function () {
    ...
    expect(testContextReports[0]).to.be.an('object')
    expect(testContextReports[0].message).to.equal(
  ...
  ;(semver.satisfies(eslintPkg.version, '>= 7.28') ? describe : describe.skip)(
    ...
    () => {
      ...
      it('resolves via a custom resolver with interface version 2', function () {
        ...
        expect(testContextReports[0]).to.be.an('object')
        expect(
          replaceErrorStackForTest(testContextReports[0].message),
        ).to.equal(
          'Resolve error: foo-bar-resolver-v2 resolve test exception\n<stack-was-here>',
        )
        expect(testContextReports[0].loc).to.eql({ line: 1, column: 0 })
  ...
  caseDescribe('case sensitivity', function () {
    ...
    it('detects case does not match FS', function () {
      expect(fileExistsWithCaseSync(file, testSettings)).to.equal(false)
    })
    it('detecting case does not include parent folder path (issue #720)', function () {
      ...
      expect(fileExistsWithCaseSync(f, testSettings)).to.equal(true)
    })
    it('detecting case should include parent folder path', function () {
      ...
      expect(fileExistsWithCaseSync(f, testSettings, true)).to.equal(false)
    })
  })
  ...
  describe('rename cache correctness', function () {
    ...
    pairs.forEach(([original, changed]) => {
      describe(`${original} => ${changed}`, function () {
        ...
        beforeEach('rename', function (done) {
          fs.rename(
        ...
        afterEach('restore original case', function (done) {
          fs.rename(
            utils.testFilePath(changed),
            utils.testFilePath(original),
            done,
