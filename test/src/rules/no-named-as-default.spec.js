...
import rule from 'rules/no-named-as-default'
...
ruleTester.run('no-named-as-default', rule, {
  valid: [].concat(
    ...
    test({ code: 'export bar from "./bar";', parser: parsers.BABEL_OLD }),

    // #566: don't false-positive on `default` itself
    test({ code: 'export default from "./bar";', parser: parsers.BABEL_OLD }),
