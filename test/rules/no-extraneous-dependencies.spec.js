...
import { dependencies, devDependencies } from '../fixtures/package.json'
import { parsers, test, testFilePath } from '../utils'

import rule from 'eslint-plugin-i/rules/no-extraneous-dependencies'
...
const packageDirWithSyntaxError = path.resolve(
...
const packageDirWithFlowTyped = path.resolve('test/fixtures/with-flow-typed')
const packageDirWithTypescriptDevDependencies = path.resolve(
  'test/fixtures/with-typescript-dev-dependencies',
)
const packageDirMonoRepoRoot = path.resolve('test/fixtures/monorepo')
const packageDirMonoRepoWithNested = path.resolve(
  'test/fixtures/monorepo/packages/nested-package',
)
const packageDirWithEmpty = path.resolve('test/fixtures/empty')
const packageDirBundleDeps = path.resolve(
  'test/fixtures/bundled-dependencies/as-array-bundle-deps',
)
const packageDirBundledDepsAsObject = path.resolve(
  'test/fixtures/bundled-dependencies/as-object',
)
const packageDirBundledDepsRaceCondition = path.resolve(
  'test/fixtures/bundled-dependencies/race-condition',
)
...
ruleTester.run('no-extraneous-dependencies', rule, {
  valid: [
    ...
    test({
      ...
      settings: {
        'i/resolver': {
          node: { paths: [path.resolve('test/files')] },
        },
      },
    }),
    test({
  ...
  invalid: [
    ...
    test({
      code: 'import "bar"',
      options: [{ packageDir: path.resolve('./doesn-exist/') }],
      errors: [
        {
    ...
    test({
      ...
      settings: {
        'i/resolver': {
          modules: [path.resolve('test/fixtures'), 'node_modules'],
        },
