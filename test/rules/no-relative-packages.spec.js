...
import rule from 'eslint-plugin-i/rules/no-relative-packages'
...
ruleTester.run('no-relative-packages', rule, {
  ...
  invalid: [
    ...
    test({
      ...
      errors: [
        {
          message: `Relative import from another package is not allowed. Use \`${path.normalize('eslint-plugin-i/test/fixtures/bar')}\` instead of \`../bar\``,
          line: 1,
          column: 17,
        },
      ],
      output: `import bar from "eslint-plugin-i/test/fixtures/bar"`,
    }),
  ],
})
