/**
 * @typedef {import('semver').Range} Range
...
/**
 * @param {string | Range} specifier
 * @param {() => Partial<InvalidTestCase>} t
 * @returns {InvalidTestCase | InvalidTestCase[]}
 */
...
export function test(t) {
  ...
  return {
    ...
    parserOptions: {
      ...
      ecmaVersion: 2023,
      babelOptions:
        t.parser === parsers.BABEL
          ? {
              configFile: require.resolve('../babel.config'),
              ...t.parserOptions?.babelOptions,
            }
          : undefined,
      ...t.parserOptions,
    },
 ...
 * @param {boolean} [getPhysicalFilename]
 * @returns {RuleContext}
 */
export function testContext(settings, getPhysicalFilename) {
  return {
    getFilename: getContextFilename,
    getPhysicalFilename: getPhysicalFilename ? getContextFilename : null,
    settings: settings || {},
  }
}
...
export function getFilename(file) {
  return path.resolve('test/fixtures', file || 'foo.js')
}
...
export const SYNTAX_CASES = [
  ...
  test({
    ...
    settings: { 'i/extensions': ['.js'] }, // breaking: remove for v2
  }),
