...
import importType, {
  ...
  isScoped,
  isAbsolute,
} from 'eslint-plugin-i/core/importType'

describe('importType(name)', function () {
  const context = testContext()
  const pathToTestFixtures = path.resolve('test/fixtures')
  ...
  it("should return 'internal' for non-builtins resolved outside of node_modules", function () {
    const pathContext = testContext({
      'i/resolver': { modules: [pathToTestFixtures, 'node_modules'] },
    })
  ...
  it("should return 'internal' for aliased internal modules that look like core modules", function () {
    const pathContext = testContext({
      'i/resolver': { modules: [pathToTestFixtures, 'node_modules'] },
    })
  ...
  it("should return 'internal' for module from 'node_modules' if 'node_modules' missed in 'external-module-folders'", function () {
    const foldersContext = {
      ...
      getFilename() {
        return path.resolve('foo.js')
      },
    }
    expect(importType('semver', foldersContext)).toEqual('internal')
  })
  ...
  it('returns "internal" for a scoped symlinked module', function () {
    const foldersContext = testContext()
  ...
  it('returns "external" for a scoped module from a symlinked directory which name is contained in "external-module-folders"', function () {
    const foldersContext = testContext({
  ...
  it('returns "internal" for a scoped module from a symlinked directory which incomplete name is contained in "external-module-folders"', function () {
  ...
  it('returns "external" for a scoped module from a symlinked directory which partial path is contained in "external-module-folders"', function () {
  ...
  it('returns "internal" for a scoped module from a symlinked directory which partial path w/ incomplete segment is contained in "external-module-folders"', function () {
    const foldersContext_1 = testContext({
      'i/external-module-folders': ['fixtures/symlinked-mod'],
    })
    expect(importType('@test-scope/some-module', foldersContext_1)).toEqual(
      'internal',
    )
  ...
  it('returns "external" for a scoped module from a symlinked directory which partial path ending w/ slash is contained in "external-module-folders"', function () {
    const foldersContext = testContext({
      'i/external-module-folders': ['symlinked-module/'],
    })
    expect(importType('@test-scope/some-module', foldersContext)).toEqual(
  ...
  it('returns "internal" for a scoped module from a symlinked directory when "external-module-folders" contains an absolute path resembling directory‘s relative path', function () {
    const foldersContext = testContext({
      'i/external-module-folders': ['/symlinked-module'],
    })
    expect(importType('@test-scope/some-module', foldersContext)).toEqual(
  ...
  it('returns "external" for a scoped module from a symlinked directory which absolute path is contained in "external-module-folders"', function () {
    const foldersContext = testContext({
